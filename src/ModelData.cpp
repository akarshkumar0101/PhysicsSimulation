...
void ModelData::loadFromFile(const std::string &fileName) {
...
std::vector<unsigned int> ModelData::defaultListOfVertices(const std::vector<float> &vertices) {
...
ModelData::ModelData(const std::vector<float> &vertices) : ModelData(vertices, defaultListOfVertices(vertices)) {
}

ModelData::ModelData(const std::vector<float> &vertices, const std::vector<unsigned int> &indices) : mVertices(
        vertices),
                                                                                                     mIndices(indices) {
    initBuffers();
}

ModelData::ModelData(const RigidModel &model) {
...
void ModelData::draw(const Shader &shader) const {
    shader.bind();
    vertexBuffer->bind();
    indexBuffer->bind();
...
void ModelData::drawModelOutline(const Shader &shader) const {
    shader.bind();
    vertexBuffer->bind();
    indexBuffer->bind();
...
void ModelData::drawModel(const RigidModel &rigidModel, const ModelData &jointModel, const Shader &shader) {
